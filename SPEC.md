...
- **A slice of regen survivors goes to human eyes.** `review.audit` (optional) badges an auto-passed regen take for the morning review pile, recording the sampling `rate` in force. Selection hashes `take_id`, so the same take always answers the same and re-reviews need no state. An audited take's verdict is `review` even when the judges found nothing; gold labels on audited takes feed `fit` and `calibrate`, and rising disagreement there is the alarm that the loop found a judge blind spot.
